...
def calcMultirotorTravelTime(takeoffSpeed, cruiseSpeed, landSpeed, yawRateDeg, initAlt, flightAlt, goalAlt, initLatDeg,
                             initLongDeg, goalLatDeg, goalLongDeg, initHeadingDeg, goalHeadingDeg):
    # Example:
    ...
    if (([initLatRad, initLongRad, initAlt] == [goalLatRad, goalLongRad, goalAlt]) and (goalHeadingDeg == -361)):
        ...
        if ([initLatRad, initLongRad] != [goalLatRad, goalLongRad]):
            if (initHeadingDeg <= -361):
                ...
                x = cos(initLatRad) * sin(goalLatRad) - sin(initLatRad) * cos(goalLatRad) * cos(
                    goalLongRad - initLongRad)
...
def groundDistanceStraight(lat1, long1, lat2, long2):
    ...
    distance = 2 * radius_of_earth * asin(
