...
def select_neighbours(current_state, drone_id):
    ...
    for i, node in enumerate(current_state):
        if i != 0 and i != (len(current_state) - 1) \
                and current_state[i - 1][0] == 1 and current_state[i + 1][0] == 1:
            ...
            if node[0] == 1:  # truck
                if i == len(sol) - 1:
                    ...
                    if last_drone_node != -1:
                        intermediary_drone_cost += distances[2][last_drone_node][node[1]].totalTime
                ...
                intermediary_drone_cost += distances[node[0]][last_truck_node][node[1]].totalTime
